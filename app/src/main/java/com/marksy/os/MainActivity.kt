...
class MainActivity : ComponentActivity() {
    ...
    private fun MarksyApp() {
        ...
        Scaffold(containerColor = Background, bottomBar = {
            if (!showTimeline && !showCalendar && !showInsights && !showRules) {
...
@Composable private fun InsightsHost(events: List<NotificationEventEntity>, padding: PaddingValues, onBack: () -> Unit) {
    Column(Modifier.fillMaxSize()) { ScreenHeader("Insights", onBack); InsightsScreen(events = events, padding = PaddingValues(bottom = padding.calculateBottomPadding())) }
}

@Composable private fun RulesHost(padding: PaddingValues, onBack: () -> Unit) {
    Column(Modifier.fillMaxSize()) { ScreenHeader("Rules & Automation", onBack); RulesScreen(PaddingValues(bottom = padding.calculateBottomPadding())) }
}
...
@Composable private fun TradingScreen(insights: List<TradingInsight>, padding: PaddingValues) {
    ...
    LazyColumn(Modifier.fillMaxSize().padding(padding), contentPadding = PaddingValues(18.dp), verticalArrangement = Arrangement.spacedBy(10.dp)) {
        ...
        else items(insights, key = { it.eventId }) { insight -> TradingInsightCard(insight) { selected = insight } }
    }
    selected?.let { TradingInsightDetailDialog(it) { selected = null } }
}

@Composable private fun TradingInsightCard(insight: TradingInsight, onClick: () -> Unit) {
    Card(colors = CardDefaults.cardColors(containerColor = Raised), modifier = Modifier.fillMaxWidth(), onClick = onClick) {
        Column(Modifier.padding(15.dp)) {
            Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween, verticalAlignment = Alignment.CenterVertically) { Text(insight.source.ifBlank { "Trading source" }, color = Primary, fontWeight = FontWeight.SemiBold); Text(insight.status.ifBlank { "Unknown" }, color = TextSecondary, fontSize = 11.sp) }
            Spacer(Modifier.height(7.dp)); Text(insight.headline.ifBlank { "Trading event" }, color = TextPrimary, fontSize = 17.sp, fontWeight = FontWeight.SemiBold)
            ...
            Spacer(Modifier.height(9.dp)); Text("Classifier ${(insight.confidence * 100).toInt()}%  •  Tap for details", color = TextMuted, fontSize = 11.sp)
        }
    }
}

@Composable private fun MoreScreen(access: Boolean, whatsappAccess: Boolean, openAccess: () -> Unit, openWhatsAppAccess: () -> Unit, clearAll: suspend () -> Unit, openTimeline: () -> Unit, openCalendar: () -> Unit, openInsights: () -> Unit, openRules: () -> Unit, padding: PaddingValues) {
    var showClear by remember { mutableStateOf(false) }; var clearing by remember { mutableStateOf(false) }; val scope = rememberCoroutineScope()
...
@Composable private fun EmptyState(title: String, message: String) {
