...
@Composable private fun GatewaySettingsHost(padding: PaddingValues, onBack: () -> Unit) {
    ...
    Column(Modifier.fillMaxSize().padding(padding)) {
        ...
        Column(Modifier.fillMaxWidth().padding(18.dp), verticalArrangement = Arrangement.spacedBy(12.dp)) {
            ...
            OutlinedTextField(value = key, onValueChange = { key = it }, modifier = Modifier.fillMaxWidth(), label = { Text("Integration key") }, singleLine = true)
            Button(onClick = {
                runCatching { store.setIntegrationKey(key) }.onSuccess { key = ""; configured = true; message = "Saved securely on this device." }.onFailure { message = "Could not save the credential." }
            }, enabled = key.isNotBlank()) { Text("Save Securely") }
            OutlinedButton(onClick = { store.clearIntegrationKey(); key = ""; configured = false; message = "Credential removed. Trading delivery is disabled until configured." }) { Text("Remove Credential") }
...
@Composable private fun MoreScreen(access: Boolean, whatsappAccess: Boolean, openAccess: () -> Unit, openWhatsAppAccess: () -> Unit, openGatewaySettings: () -> Unit, clearAll: suspend () -> Unit, openTimeline: () -> Unit, openCalendar: () -> Unit, openInsights: () -> Unit, openRules: () -> Unit, padding: PaddingValues) { var showClear by remember { mutableStateOf(false) }; var clearing by remember { mutableStateOf(false) }; val scope = rememberCoroutineScope(); val gatewayConfigured = remember { SecureCredentialStore(AppContext.get()).getIntegrationKey() != null } ; LazyColumn(Modifier.fillMaxSize().padding(padding), contentPadding = PaddingValues(18.dp), verticalArrangement = Arrangement.spacedBy(12.dp)) { item { Text("More", color = TextPrimary, fontSize = 28.sp, fontWeight = FontWeight.Bold); Spacer(Modifier.height(6.dp)); Text("Marksy OS settings and local data controls.", color = TextSecondary, fontSize = 13.sp) }; item { SettingsCard("Notification access", if (access) "ON" else "OFF", if (access) "Marksy OS can capture notifications." else "Enable notification access to start capturing.") { Button(onClick = openAccess) { Text(if (access) "Manage Access" else "Open Access") } } }; item { SettingsCard("WhatsApp connector", if (whatsappAccess) "ON" else "OPTIONAL", "Reads visible WhatsApp accessibility text only for senders on your local allow-list.") { Button(onClick = openWhatsAppAccess) { Text(if (whatsappAccess) "Manage Connector" else "Set Up Connector") } } }; item { SettingsCard("Marksy Gateway", if (gatewayConfigured) "READY" else "NOT CONFIGURED", "Integration credentials are stored securely on this device. Live brokerage execution is disabled in V1.") { Button(onClick = openGatewaySettings) { Text("Configure Gateway") } } }; item { SettingsCard("Insights", "LOCAL", "Review notification patterns, attention levels, and trading intelligence delivery health.") { Button(onClick = openInsights) { Text("Open Insights") } } }; item { SettingsCard("Rules & Automation", "LOCAL", "Create deterministic local rules for highlighting and prioritizing events. Brokerage execution is not available.") { Button(onClick = openRules) { Text("Open Rules") } } }; item { SettingsCard("Timeline", "LOCAL", "Review meaningful events chronologically.") { Button(onClick = openTimeline) { Text("Open Timeline") } } }; item { SettingsCard("Calendar", "LOCAL", "Browse retained notification history by day.") { Button(onClick = openCalendar) { Text("Open Calendar") } } }; item { SettingsCard("Local data", "7d / 30d", "Ordinary notifications expire after 7 days; trading events are retained for 30 days.") { Button(onClick = { showClear = true }, enabled = !clearing) { Text(if (clearing) "Clearing…" else "Clear All Local Data") } } } }; if (showClear) AlertDialog(onDismissRequest = { if (!clearing) showClear = false }, title = { Text("Clear local data?") }, text = { Text("This removes captured notifications and trading intelligence stored on this device. It cannot be undone.") }, confirmButton = { TextButton(enabled = !clearing, onClick = { clearing = true; scope.launch { try { clearAll() } finally { clearing = false; showClear = false } } }) { Text("Clear") } }, dismissButton = { TextButton(enabled = !clearing, onClick = { showClear = false }) { Text("Cancel") } }) }
