...
fun SmartInboxScreen(
    ...
    val filtered = remember(events, filter) { SmartInboxModel.filter(events, filter) }
            ...
            Row(Modifier.horizontalScroll(rememberScrollState()), horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                SmartInboxModel.Filter.entries.forEach { item ->
                    FilterChip(
                        ...
                        onClick = { selectedFilterName = item.name },
...
private fun LazyListScope.inboxSection(
    title: String,
    threads: List<SmartInboxModel.Thread>,
    onEventSelected: (NotificationEventEntity) -> Unit
) {
...
private fun InboxThreadCard(thread: SmartInboxModel.Thread, onEventSelected: (NotificationEventEntity) -> Unit) {
    ...
    val attentionLabel = when (thread.attentionLevel) {
        ...
        EventIntelligence.AttentionLevel.LOW -> "Low attention"
    }
    Card(
        colors = CardDefaults.cardColors(containerColor = if (thread.highestAttention >= 70) InboxRaised else InboxSurface),
        ...
        Column(Modifier.padding(14.dp)) {
            Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween) {
                Text(
                    ...
                    color = if (event.isTrading) InboxPrimary else InboxSecondary,
                ...
                Text(
                    "$attentionLabel · ${thread.highestAttention}/100",
                    color = InboxPrimary,
                    fontSize = 11.sp,
            ...
            if (thread.hasDeliveryFailure) Text("Trading delivery failed — action needed", color = InboxPrimary, fontSize = 11.sp, modifier = Modifier.padding(top = 4.dp))
            else if (event.isTrading && event.deliveryState != DeliveryState.NOT_APPLICABLE.name) Text(inboxDeliveryLabel(event.deliveryState), color = InboxPrimary, fontSize = 11.sp, modifier = Modifier.padding(top = 4.dp))
        }
    }
}
