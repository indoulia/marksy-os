...
object PromptTemplates {
    val ASK_INTERPRET = PromptTemplate(
        ...
        text = """
            ...
            "range": one of today|yesterday|tomorrow|this_week|last_week|this_month|default,
            "subject": person, app or search words or null, "direction": DEBIT|CREDIT|null, "confidence": 0..1}.
        ...
        schema = JsonSchema(
            mapOf(
                ...
                "range" to JsonSchema.FieldSpec(JsonSchema.Type.STRING, enum = setOf("today", "yesterday", "tomorrow", "this_week", "last_week", "this_month", "default")),
...
class ModelQueryInterpreter(private val service: IntelligenceService) : AskMarksy.QueryInterpreter {
    ...
    internal fun toQuery(o: JSONObject, text: String, previous: AskMarksy.Query?, nowMillis: Long, zone: ZoneId): AskMarksy.Query {
        ...
        val subject = if (o.has("subject") && !o.isNull("subject")) o.getString("subject").trim().lowercase().take(60).ifBlank { null } else null
